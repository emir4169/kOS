...
keyboard_cb(__attribute__((unused)) i_register_t registers) 
{
    ...
    switch (scan)
    {
        case _LSHIFT: //LSHIFT
        case _RSHIFT: //RSHIFT
            caps_lock = !caps_lock; // this seems wrong but it does work
            break;
        case _CAPS: //CAPS
            caps_lock = pressed ? !caps_lock : caps_lock; // this seems wrong but it does work
            break;
    }

    // make sure the stuff that SHOULDNT be written to the screen, doesnt.
    // RIP the old switch case statement for this 2025-2025
    KASSERT_GOTO_SUCCESS(
        !((
            ( // upper bound
                (
                    caps || caps_lock
                )
                ?   uppercase[scan] : lowercase[scan]
            )
            < 128
        ) && (
            ( // lower bound
